...
const weightTrainingSingleClass = 1500;
let AlexBudget = 3000;
...
let totalCost = weightTrainingGroupClass + yogaGroupClass * 3 + weightTrainingSingleClass
...
// 方案名稱
const planName = '14堂組合包方案';
// 方案價格
let planPrice = 2520;
// 課堂數量
let classesNum = 14;
// 平均每堂價格
let averageClassesPrice = planPrice / classesNum; 

// ### 題目五：布林值與變數定義，看是否有用對 const, let
// 情境：Alex 在往健身房的路上，望向城市的風景，請描述她看到的一切，並宣告變數與賦予值
// 4-1. Alex 在等紅綠燈，他抬頭看一下現在是紅燈，還有 28 秒綠燈（最多 3 個宣告）
let lightColor = 'red';
let lightRemainingSeconds = 28;
// 是否已通過
let isPassed = false;
// 4-2. 目前一起等待的機車有 8 台
let totalWaitingMotorcycleNum = 8;
...
a = 13; // 將一個 13 的數字型別重新賦予 a
a = b + 4; // a = 4   // 將一個 b 變數(0) + 一個 4 的數字型別重新賦予 a
a - b;     // 4       // a 變數(4) - b 變數(0)，僅為計算結果
b += 1;    // b = 1   // 將 b 變數(0) + 一個 1 的數字型別重新賦予 b
